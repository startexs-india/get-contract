"use client";
import React from "react";

export default function RequiredAttachmentsSection({ data = [], editMode, onChange }) {
    if (!Array.isArray(data) || data.length === 0) return null;

    return (
        <div className="mt-6">
            <h2 className="text-xl font-semibold mb-3">Required Attachments</h2>

            {data.map((item, idx) => (
                <div
                    key={idx}
                    className="border border-gray-300 rounded mb-3 grid grid-cols-1 md:grid-cols-2"
                >
                    {["label", "isMandatory", "description"].map((field) => (
                        <div key={field} className="border p-3 flex gap-4">
                            <div className="text-sm font-semibold text-gray-600 min-w-[140px]">
                                {field.replace(/([A-Z])/g, " $1").toUpperCase()}
                                <span className="text-black font-semibold text-lg ml-1">:</span>
                            </div>

                            {!editMode ? (
                                <div className="text-gray-800">
                                    {field === "isMandatory"
                                        ? item[field]
                                            ? "Yes"
                                            : "No"
                                        : item[field] ?? "N/A"}
                                </div>
                            ) : field === "isMandatory" ? (
                                <select
                                    className="border-2 border-gray-300 rounded px-2 py-1 w-full"
                                    value={item[field] ? "true" : "false"}
                                    onChange={(e) =>
                                        onChange(idx, field, e.target.value === "true")
                                    }
                                >
                                    <option value="true">Yes</option>
                                    <option value="false">No</option>
                                </select>
                            ) : (
                                <input
                                    className="border-2 border-gray-300 rounded px-2 py-1 w-full"
                                    value={item[field] ?? ""}
                                    onChange={(e) =>
                                        onChange(idx, field, e.target.value)
                                    }
                                />
                            )}
                        </div>
                    ))}
                </div>
            ))}
        </div>
    );
}
